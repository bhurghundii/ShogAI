...
full_data_black = pd.read_csv('black_training_3.csv')
full_data_white = pd.read_csv('white_training_3.csv')
...
autoencoder_black.save('autoencoder.h5')
...
h = Dense(800, activation='relu', name='hidden', kernel_regularizer=regularizers.l2(0.00005))(inputTensorForMlp)
#keep increasing dropot
h2 = Dropout(0.4)(h)
h3 = Dense(400, activation='relu', name='hidden1', kernel_regularizer=regularizers.l2(0.00005))(h2)
h4 = Dropout(0.4)(h3)
h5= Dense(200, activation='relu', name='hidden2', kernel_regularizer=regularizers.l2(0.00005))(h4)

out = Dense(1, activation='sigmoid', name='prediction')(h5)

model = Model(inputs=inputTensorForMlp, outputs=out)
adamOpt = Adam(lr=0.0001, decay=0.000000005)

model.compile(loss='binary_crossentropy', optimizer=adamOpt, metrics=['accuracy'])

#5000 for final
model.fit(inputDataForThird ,third_train_y , epochs=1000, shuffle=True, batch_size=32)
...
#This is really optimized for white (accident) but no biggie. 
b = 1-labels

target_names = ['Win', 'Loss']
print(accuracy_score(third_test_y, b))
